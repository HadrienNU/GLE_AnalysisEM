...
from .model_class import AbstractModel


class ABOBA(AbstractModel):
    def __init__(self, dim_x):
        AbstractModel.__init__(dim_x)

    def preprocessingTraj(X, idx_trajs=[], dim_x=1):
        """
    ...
    def compute_expectation_estep_aboba(traj, expA, force_coeffs, dim_h, dt):
        """
    ...
    def negloglike_tominimize(theta, suff_datas, dim_x, dim_h, dt):
        ...
        return -loglikelihood(suff_datas, expA, SST, coeffs_force, dim_h, dt)
    ...
    def m_step_num(expA, SST, coeffs_force, sufficient_stat, dim_h, dt, EnforceFDT, OptimizeDiffusion, OptimizeForce):
        """
    ...
    def loglikelihood(suff_datas, expA, SST, coeffs_force, dim_x, dim_h, dt):
        """
    ...
    def ABOBA_generator(nsteps=50, dt=5e-3, dim_x=1, dim_h=1, x0=None, v0=None, friction=None, SST=None, force_coeffs=None, muh0=0.0, sigh0=0.0, basis=None, rng=np.random.default_rng()):
        """
        ...
        for n in range(1, nsteps):
            ...
            phalfprime = np.matmul(friction[0:dim_x, 0:dim_x], phalf) + np.matmul(friction[0:dim_x, dim_x:], h_traj[n - 1, :]) + gauss[:dim_x]
            h_traj[n, :] = np.matmul(friction[dim_x:, 0:dim_x], phalf) + np.matmul(friction[dim_x:, dim_x:], h_traj[n - 1, :]) + gauss[dim_x:]
