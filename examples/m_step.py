...
est = GLE_Estimator(init_params="user", dim_x=dim_x, dim_h=dim_h, model=model, EnforceFDT=True, OptimizeDiffusion=True)
est_euler = GLE_Estimator(init_params="user", dim_x=dim_x, dim_h=dim_h, model="euler", EnforceFDT=True, OptimizeDiffusion=True)
...
Xproc, idx = est.model.preprocessingTraj(X, idx_trajs=idx)
