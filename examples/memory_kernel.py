...
estimator = GLE_Estimator(verbose=1, C_init=np.identity(2))
estimator.fit(X)
time, kernel = memory_kernel(150, 5e-3, estimator._get_parameters(), 1)
time_true, kernel_true = memory_kernel(150, 5e-3, {"C": np.identity(2), "A": np.array([[5, 1.0], [-2.0, 0.07]])}, 1)

plt.plot(time, kernel[:, 0, 0])
plt.plot(time_true, kernel_true[:, 0, 0])
plt.show()
