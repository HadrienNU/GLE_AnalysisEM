...
est = GLE_Estimator(init_params="user", dim_x=dim_x, dim_h=dim_h, model=model, OptimizeDiffusion=True, random_state=None, max_iter=15, A_init=[[5.5, 0.8], [-1.0, 0.08]], force_init=force)
...
print(datas_true)
to_plot_logL_true_true, _ = generator.loglikelihood(datas_true)  # The true likelihood of the true datas
...
to_plot_logL = np.zeros((est.max_iter, 4))
to_plot_logL_true_datas[0], _ = est.loglikelihood(datas_true)  # The initial likelihood on the true datas
...
for n_iter in range(est.max_iter):
    ...
    to_plot_logL[n_iter, 0], to_plot_logL[n_iter, 2] = est.loglikelihood(new_stat)  # + new_stat["hS"]  # The initial likelihood
    to_plot_logL_true_param[n_iter], _ = generator.loglikelihood(new_stat)  # + new_stat["hS"]  # The initial datas of the true param

    # print("M")
    est._m_step(new_stat)
    to_plot_logL[n_iter, 1], to_plot_logL[n_iter, 3] = est.loglikelihood(new_stat)  # + new_stat["hS"]  # After M step
    to_plot_logL_true_datas[n_iter + 1], _ = est.loglikelihood(datas_true)  # After M step on the true datas
print(new_stat)
...
plt.plot(to_plot_logL[:, 1], label="After Mstep")
